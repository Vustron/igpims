...
interface EditIgpFormProps {
  igpId: string
  initialData?: Partial<UpdateIgpPayload>
  onSuccess?: () => void
  onError?: () => void
  onClose?: () => void
}

export const EditIgpForm = ({
  igpId,
  initialData,
  onSuccess,
  onError,
  ...
  const form = useForm<UpdateIgpPayload>({
    resolver: zodResolver(updateIgpSchema),
    defaultValues: {
      igpName: initialData?.igpName || "",
      igpDescription: initialData?.igpDescription || "",
      iconType: initialData?.iconType || "",
      semesterAndAcademicYear: initialData?.semesterAndAcademicYear || "",
      igpType: initialData?.igpType || "",
      igpStartDate: initialData?.igpStartDate || new Date(),
      igpEndDate:
        initialData?.igpEndDate ||
        new Date(new Date().setMonth(new Date().getMonth() + 1)),
      itemsToSell: initialData?.itemsToSell || "",
      ...
      igpDateNeeded: initialData?.igpDateNeeded || new Date(),
    },
  })
  useEffect(() => {
  ...
  const editIgpFields: FieldConfig<UpdateIgpPayload>[] = [
    // Changed to UpdateIgpPayload
    {
      name: "projectLead",
      type: "select",
      label: "Project Lead",
  ...
  const onSubmit = async (values: UpdateIgpPayload) => {
    try {
      ...
      await toast.promise(updateIgp.mutateAsync(formData), {
        // Changed to updateIgp
        loading: <span className="animate-pulse">Updating proposal...</span>, // Changed message
        success: "Proposal successfully updated", // Changed message
        error: (error: unknown) => catchError(error),
      })

      if (updateIgp.isSuccess) {
        // Changed to updateIgp
        setIsSubmitted(true) // Set submitted state for success screen
        setSubmittedProjectId(igpId) // Use igpId for success screen
        onSuccess?.()
      } else {
        onError?.()
      }
      ...
      toast.error(errorMessage || "Failed to update IGP proposal") // Changed message
    }
  }
      ...
      <div className="flex flex-col items-center justify-center space-y-6 p-8 text-center">
        ...
        <div className="space-y-2">
          ...
          <p className="max-w-md text-gray-500 text-sm">
            ...
            updated and changes have been saved.
          </p>
        </div>
  ...
  return (
    <div className="h-[80vh] overflow-y-auto p-6">
      <DynamicForm
        form={form}
        onSubmit={onSubmit}
        fields={editIgpFields}
        submitButtonTitle={isSubmitting ? "Updating..." : "Update IGP Proposal"}
        mutation={updateIgp}
